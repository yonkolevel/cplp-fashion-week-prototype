...
export const Hero = ({ data }: { data: PageBlocksHero }) => {
  ...
  return (
    <Section background={data.background!} className='cplp-hero py-20 md:py-28'>
          ...
          {data.actions &&
            data.actions.map((action) => (
              <div key={action!.label} data-tina-field={tinaField(action)} className='bg-foreground/10 rounded-[calc(var(--radius-xl)+0.125rem)] border p-0.5'>
                <Button asChild size='lg' variant={action!.type === 'link' ? 'ghost' : 'default'} className={`rounded-xl px-5 text-base ${action!.type === 'link' ? 'text-white hover:bg-white/10 hover:text-white' : ''}`}>
